...
class Data_Loader(object):
    ...
    def load_session_group_user_query_item_label_hiera(
        self,
        data_path,
        word_index,
        query_word_index,
            ...
            while True:
                ...
                if not line:
                    if len(X_Q) > 0:
                        ...
                        new_X_Q = list(
                            map(lambda x: pad_sequences(x, query_maxlen), X_Q))
                        new_X = list(
                            map(
                                lambda x: pad_sequences(
                                    list(
                                        map(
                                            lambda y: pad_sequences(
                                                y, title_maxlen), x)),
                                    title_maxsize), X))
                ...
                for pv in session:
                    ...
                    query_maxlen = len(query_id) if len(
                        query_id) > query_maxlen else query_maxlen
                    ...
                    for title in items:
                        ...
                        title_maxlen = len(title_id) if len(
                            title_id) > title_maxlen else title_maxlen
                    title_maxsize = len(title_tmp) if len(
                        title_tmp) > title_maxsize else title_maxsize
                    title_seq.append(title_tmp)
                query_maxsize = len(title_seq) if len(
                    title_seq) > query_maxsize else query_maxsize
    ...
    def load_session_group_user_query_item_label_inference(
        self,
        data_path,
        word_index,
        query_word_index,
        ...
        **kwargs,
        ...
        with open(data_path, 'r') as fp:
            while True:
                ...
                if not line:
                    ...
                    for k, v in item_feature_dict.items():
                        ...
                        try:
                            ...
                            new_X_Q = pad_sequences(
                                list(
                                    map(
                                        lambda x: pad_sequences(
                                            x, query_maxlen), v[0][2])),
                                query_maxsize)

                            new_X_Y = to_categorical(v[0][3],
                                                     num_classes=nb_classes)
                            new_item_feature_dict[k] = [[
                                new_Item, new_X, new_X_Q, new_X_Y
                            ], v[1], v[2]]
                        except:
                            num += 1
                            # print(v)
                ...
                query_maxsize = len(query_seq) if len(
                    query_seq) > query_maxsize else query_maxsize
                ...
                if item_id in item_feature_dict and label == 'black_list':
                    ...
                    item_feature_dict[item_id] = [[[item_title], [title_seq],
                                                   [query_seq], [[1]]], 1,
                                                  title]
    ...
    def session_group_user_query_item_generator_hiera(
        self,
        data_path,
        word_index,
        query_word_index,
        ...
        **kwargs,
        ...
        fp = open(data_path, 'r', encoding='utf8')
        ...
        while True:
            ...
            for pv in session:
                ...
                query_seq.append(query_id)
                query_maxlen = len(
                    query_id) if len(query_id) > query_maxlen else query_maxlen
            ...
            query_maxsize = len(
                query_seq) if len(query_seq) > query_maxsize else query_maxsize
            ...
            if count == batch_size:
                ...
                new_X = list(
                    map(
                        lambda x: pad_sequences(
                            list(
                                map(lambda y: pad_sequences(y, title_maxlen), x
                                    )), title_maxsize), X))
...
def main():
    ...
    parser.add_argument('--phase',
                        default='read_data',
                        help='Please enter what function you will use.')
